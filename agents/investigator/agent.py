...
@tool
def invoke_aws_api(region: str, service: str, operation: str, parameters: str = "{}") -> Dict[str, Any]:
    """
    Invoke AWS API operations dynamically. Only allows read-only operations.
    
    Args:
        role_arn (str): The ARN of the IAM role to assume for the API call
        region (str): The AWS region to use for the API call
    ...
    try:
        ...
        if not region:
            return {"error": "Region must be specified for invoking AWS API operations."}
        if not service:
        ...
        client = boto3.client(service, region=region)
